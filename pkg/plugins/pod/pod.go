...
const (
	...
	STAGE_3_ALLOW_RUNNING_DEPENDS_ON_WAITING_PODS = 25
	STAGE_4_ALLOW_RUNNING_DEPENDS_ON_WAITING_PODS = 35
	STAGE_5_ALLOW_RUNNING_DEPENDS_ON_WAITING_PODS = 45
	STAGE_6_ALLOW_RUNNING_DEPENDS_ON_WAITING_PODS = 45
...
func AllowWaitingPodAfterTime(sampleName string, handle framework.FrameworkHandle, samplePods map[string]*SamplePod) {
	...
	time.AfterFunc(time.Duration(STAGE_6_ALLOW_RUNNING_DEPENDS_ON_WAITING_PODS)*time.Second, func() {
		AllowWaitingPods(sampleName, handle, samplePods)
	})
}
